...
export function RecipeBrowser({
  ...
  const [recipes, setRecipes] = useState<any[]>(initialData || []);
  const [activeLetter, setActiveLetter] = useState("a");
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  // Keep recipes synced when parent updates
  useEffect(() => {
  ...
  const handleFilterSelect = async (filterName: string) => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/meals/filter`, {
        ...
        body: JSON.stringify({ type: filterName }),
      });

      const data = await res.json();
      const mealArray =
        data?.meals ? data.meals : Array.isArray(data) ? data : [];
            ...
            {isFilterOpen && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-md shadow-lg z-20 py-1">
                  {["Vegetarian", "Vegan", "Breakfast", "Dessert"].map(
                    (label) => (
                      <button
                        key={label}
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-3 gap-4">
            {recipes.map((recipe, index) => {
              const recipeName =
                recipe?.name || recipe?.strMeal || "Unknown Recipe";
              const recipeImage =
                recipe?.image ||
                recipe?.strMealThumb ||
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {recipe?.displayedIngredients?.map(
        ...
        <div className="flex flex-wrap gap-1.5 max-w-full">
          {alphabet.map((letter) => (
            <Button
              key={letter}
              variant={
                activeLetter === letter ? "default" : "outline"
              }
              size="sm"
              className={`uppercase w-8 h-8 p-0 text-xs ${
                activeLetter === letter
